...
Future<Response> _handleRestRequests(
  ...
  if (path == '') {
    return Response.ok(
      '<h1>🍰 Revani Database</h1><p>The oven is on and smells wonderful. Developed by JeaFriday, for humanity!</p><hr><p><b>TCP:</b> ${RevaniConfig.port} | <b>REST:</b> ${RevaniConfig.port + 1}</p>',
  ...
  try {
    ...
    if (RevaniConfig.sslEnabled) {
      if (!File(RevaniConfig.certPath).existsSync() ||
        ...
        mainSendPort.send(
          '[!] MISSING INGREDIENT: SSL Certificates (crt/key) not found at defined paths.',
        );
        return;
      }
